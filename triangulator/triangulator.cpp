...
QList<TriangleF> Triangulator::triangulate(const QSet<QPointF> &points)
{
	QList<TriangleF> triangles;
...
//			qDebug() << edge;
		...
		for (int j = 0; j < edges.count(); ++j) {
			triangles.append(TriangleF(edges.at(j).p1(), edges.at(j).p2(), p));
...
//	qDebug() << "Triangles Before" << triangles.count();
	for (int i = 0; i < triangles.count(); ++i) {
		const TriangleF &t = triangles.at(i);
...
TriangleF::TriangleF(const QPointF &p1, const QPointF &p2, const QPointF &p3)
	: v1(p1)
	, v2(p2)
	, v3(p3)
{}
